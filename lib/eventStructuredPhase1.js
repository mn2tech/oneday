...
function normalizePhotoSubsection(item, idx) {
  const idRaw = typeof item?.id === 'string' ? item.id.trim() : '';
  return {
    id: (idRaw || `photo_${idx + 1}`).slice(0, 64),
    title: cleanText(item?.title, MAX_FIELD.photoSubTitle),
  };
}
...
export function normalizePhase1Content(input, fallbackTitle = '') {
  ...
  const photoWall = src.photoWall && typeof src.photoWall === 'object' ? src.photoWall : {};
  const photoSubsections = Array.isArray(photoWall.subsections) ? photoWall.subsections : [];
  ...
  return {
    ...
    photoWall: {
      title: cleanText(photoWall.title, MAX_FIELD.photoWallTitle),
      subsections: photoSubsections.slice(0, 12).map(normalizePhotoSubsection),
    },
  };
}

export function extractLivePhotoWallFromHtml(html) {
  const src = String(html || '');
  const lines = htmlToLines(src);
  const title = pickFirst(lines, [
    (line) => (/photo wall|photo gallery|gallery|memories|moments/i.test(line) ? line : ''),
  ]);

  const lower = src.toLowerCase();
  const controlRe = /(add\s+photos|upload\s+photos|share\s+(?:photos|memories)|add\s+photo|upload\s+photo)/g;
  const subsections = [];
  let m;
  while ((m = controlRe.exec(lower)) !== null && subsections.length < 12) {
    const pos = m.index;
    const start = Math.max(0, pos - 1400);
    const chunk = src.slice(start, pos);
    const headingRe = /<(h2|h3|h4|strong|p|div|span)[^>]*>([\s\S]*?)<\/\1>/gi;
    let hm;
    let last = '';
    while ((hm = headingRe.exec(chunk)) !== null) {
      const tx = stripTags(hm[2]);
      if (!tx) continue;
      if (/add\s+photos?|upload\s+photos?|share\s+(photos|memories)/i.test(tx)) continue;
      if (tx.length > 2 && tx.length <= MAX_FIELD.photoSubTitle) last = tx;
    }
    if (last) {
      const normalized = cleanText(last, MAX_FIELD.photoSubTitle);
      if (!subsections.some((s) => s.title.toLowerCase() === normalized.toLowerCase())) {
        subsections.push({ id: `photo_${subsections.length + 1}`, title: normalized });
      }
    }
  }

  return {
    title: cleanText(title, MAX_FIELD.photoWallTitle),
    subsections,
  };
}

export function mergePhotoWallWithFallback(primary, fallback) {
  const p = primary && typeof primary === 'object' ? primary : {};
  const f = fallback && typeof fallback === 'object' ? fallback : {};
  const pSubs = Array.isArray(p.subsections) ? p.subsections : [];
  const fSubs = Array.isArray(f.subsections) ? f.subsections : [];
  const outSubs = (pSubs.length ? pSubs : fSubs).slice(0, 12).map(normalizePhotoSubsection);
  return {
    title: cleanText(p.title || f.title || '', MAX_FIELD.photoWallTitle),
    subsections: outSubs,
  };
}
...
export function validatePhase1Content(content) {
  ...
  if (content.photoWall != null && typeof content.photoWall !== 'object') {
    errors.push('photoWall must be an object.');
  }
  if (content.photoWall && !Array.isArray(content.photoWall.subsections)) {
    errors.push('photoWall.subsections must be an array.');
  }
  return errors;
}
