...
function platformEventCreatedRecipient() {
  return normalizeNotificationEmail(
    process.env.EVENT_CREATED_NOTIFICATION_EMAIL ||
      process.env.ADMIN_NOTIFICATION_EMAIL ||
      process.env.ONEDAY_OWNER_EMAIL ||
      ''
  );
}
...
export async function sendEventCreatedNotification({ event, publicUrl }) {
  const to = platformEventCreatedRecipient();
  if (!to) {
    return { status: 'skipped', reason: 'EVENT_CREATED_NOTIFICATION_EMAIL_MISSING' };
  }

  const key = process.env.RESEND_API_KEY;
  if (!key) {
    console.warn('[notifications] Event-created email skipped: set RESEND_API_KEY.');
    return { status: 'skipped', reason: 'RESEND_NOT_CONFIGURED' };
  }

  const eventTitle = String(event?.title || 'New OneDay event').trim().slice(0, 200) || 'New OneDay event';
  const eventId = String(event?.id || '').trim();
  const hostEmail = normalizeNotificationEmail(event?.email) || 'Not provided';
  const tier = String(event?.tier || event?.plan || '').trim() || 'Unknown';
  const plan = String(event?.plan || '').trim() || 'Unknown';
  const url = publicUrl || eventUrl(eventId);
  ...
  try {
    const response = await fetch(RESEND_EMAIL_API, {
      ...
      body: JSON.stringify({
        from: notificationFromAddress(),
        to,
        subject: `New OneDay event created: ${eventTitle}`,
        html: `
          <div style="font-family:Inter,system-ui,sans-serif;max-width:560px;margin:0 auto;padding:32px 24px;background:#0a0a0f;color:#f0f0f5;border-radius:12px;">
            ...
            <h1 style="font-size:1.45rem;margin:0 0 12px;">New event created</h1>
            <p style="color:#aaa;margin:0 0 20px;line-height:1.5;">A new OneDay event page was generated and saved.</p>
            <div style="background:#151525;border:1px solid #2a2a45;border-radius:10px;padding:16px;margin-bottom:18px;">
              <p style="margin:0 0 8px;"><strong>Title:</strong> ${escapeHtml(eventTitle)}</p>
              <p style="margin:0 0 8px;"><strong>Event ID:</strong> ${escapeHtml(eventId)}</p>
              <p style="margin:0 0 8px;"><strong>Host email:</strong> ${escapeHtml(hostEmail)}</p>
              <p style="margin:0 0 8px;"><strong>Plan:</strong> ${escapeHtml(plan)}</p>
              <p style="margin:0;"><strong>Tier:</strong> ${escapeHtml(tier)}</p>
            </div>
            ${
              url
                ? `<a href="${escapeHtml(url)}" style="display:inline-block;background:linear-gradient(135deg,#7c3aed,#a855f7);color:#fff;text-decoration:none;padding:14px 26px;border-radius:8px;font-weight:600;margin:16px 0 12px;">Open event page</a>
                   <p style="background:#1a1a2e;padding:12px 16px;border-radius:6px;font-size:0.88rem;word-break:break-all;color:#c084fc;margin:0;">${escapeHtml(url)}</p>`
                : ''
            }
          </div>`,
      }),
    ...
    console.error('[notifications] Event-created email failed:', err?.message);
    return { status: 'failed', reason: err?.message || 'RESEND_SEND_FAILED' };
  }
}
