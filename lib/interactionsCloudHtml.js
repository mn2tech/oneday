...
(function(){
  function run(){
    ...
    function photoHostQs(){
      var t=typeof window!=='undefined'&&window.__ONEDAY_HOST_TOKEN__;
      return t?('&hostToken='+encodeURIComponent(t)):'';
    }

    function findPhotoSections(){
      var found=[];
      var seen=new Set();
      ...
      // 1. By ID/class patterns
      document.querySelectorAll(
        '[id*="photo"],[id*="Photo"],[id*="gallery"],[id*="Gallery"],[id*="memories"],[id*="Memories"],[id*="moments"],[id*="Moments"],'+
        '[class*="photo-wall"],[class*="photoWall"],[class*="photo-section"],[class*="gallery-section"],[class*="memories"],[class*="moments"]'
      ).forEach(function(el){
        addSec(el.closest('section')||el.closest('div[id]')||el);
      });

      // 2. By file inputs (any file input on the page)
      document.querySelectorAll('input[type="file"]').forEach(function(inp){
        addSec(inp.closest('section')||inp.closest('div[id]')||inp.parentElement);
      });

      // 3. By heading text content — find any section/div whose h2/h3 mentions photo/gallery/memories/moments
      document.querySelectorAll('section, div[id]').forEach(function(el){
        if(seen.has(el)) return;
        var h=el.querySelector('h1,h2,h3,h4');
        if(h){
          var t=(h.textContent||'').toLowerCase();
          if(/photo|gallery|memor|moment|picture|image/.test(t)) addSec(el);
        }
      });
    ...
    function getSectionIndex(sec, allSecs){
      // Try to extract index from file input ID (photo-input-0, photo-input-1, etc)
      var inp=sec.querySelector('input[type="file"]');
      if(inp){
        var m=(inp.id||'').match(/(\d+)$/);
        if(m) return parseInt(m[1],10);
      }
      // Try section id number
      var sid=(sec.id||'');
      var sm=sid.match(/(\d+)$/);
      if(sm) return parseInt(sm[1],10);
      // Fall back to position in list
      return allSecs.indexOf(sec);
    }
    ...
    function renderCloudPhotos(sec, photos, isHost){
      function hideDuplicateEmptyCopy(hasPhotos){
        var root=sec.closest('section')||sec;
        if(!root) return;
        root.querySelectorAll('p,div,span,em,strong,small,i').forEach(function(el){
          if(el.classList&&el.classList.contains('oneday-photo-grid')) return;
          var tx=(el.textContent||'').trim().toLowerCase();
          if(!tx) return;
          if(
            tx.indexOf('no photos yet')!==-1 ||
            tx.indexOf('be the first to')!==-1 ||
            tx.indexOf('capture the fun')!==-1 ||
            tx.indexOf('be the first to share')!==-1
          ){
            // Keep our managed empty-state inside the injected grid as the single source.
            if(!el.closest('.oneday-photo-grid')) el.style.display='none';
          }
          if(hasPhotos && /share your/i.test(tx)) {
            if(!el.closest('.oneday-photo-grid')) el.style.display='none';
          }
        });
      }
      // Find or create grid container
      var grid=sec.querySelector('.oneday-photo-grid');
      if(!grid){
        grid=document.createElement('div');
        grid.className='oneday-photo-grid';
        grid.style.cssText='display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:8px;margin-top:12px;';
        // Insert before the upload label or at end of section
        var lbl=sec.querySelector('label[for*="photo"],label.upload-btn,.upload-btn');
        if(lbl) lbl.parentNode.insertBefore(grid, lbl);
        else sec.appendChild(grid);
      }
      grid.innerHTML='';
      if(!photos||!photos.length){
        hideDuplicateEmptyCopy(false);
        grid.innerHTML='<p style="opacity:.55;font-size:13px;grid-column:1/-1;text-align:center;padding:16px 0;">No photos yet — be the first to add one!</p>';
        return;
      }
      hideDuplicateEmptyCopy(true);
      var viewer=ensurePhotoViewer();
      var viewerItems=photos.map(function(p,ix){
        return {url:p.url,name:'oneday-photo-'+(ix+1)+'.jpg'};
      });
      photos.forEach(function(p, photoIdx){
        var wrap=document.createElement('div');
        wrap.style.cssText='position:relative;aspect-ratio:1;border-radius:8px;overflow:hidden;background:#f0f0f0;cursor:zoom-in;';
        ...
        dl.textContent='⬇';
        dl.title='Download';
        dl.style.cssText='position:absolute;top:4px;left:4px;background:rgba(0,0,0,0.7);color:#fff;border:none;border-radius:50%;width:24px;height:24px;font-size:12px;cursor:pointer;line-height:1;display:flex;align-items:center;justify-content:center;padding:0;';
        dl.onclick=function(ev){
          ev.stopPropagation();
          quickDownload(p.url,'oneday-photo-'+(photoIdx+1)+'.jpg');
        };
        wrap.appendChild(img);
        wrap.appendChild(dl);
        wrap.onclick=function(){ viewer.open(viewerItems, photoIdx); };
        if(p.owned_by_me||isHost){
          var del=document.createElement('button');
          del.textContent='×';
          del.style.cssText='position:absolute;top:4px;right:4px;width:22px;height:22px;border-radius:50%;border:none;background:rgba(0,0,0,.55);color:#fff;font-size:14px;line-height:1;cursor:pointer;display:flex;align-items:center;justify-content:center;padding:0;';
          del.onclick=function(ev){
            ...
            fetch('/api/event-photos/delete',{
              method:'POST',
              headers:{'Content-Type':'application/json'},
              body:JSON.stringify({photoId:p.id,eventId:eid,deviceId:getDeviceId()})
            }).then(function(r){return r.json();}).then(function(){
              loadCloudPhotos();
            }).catch(function(e){alert(e.message||'Could not delete photo');});
          };
    ...
    function loadCloudPhotos(){
      var allSecs=findPhotoSections();
      if(!allSecs.length) return;
      fetch('/api/event-photos/list?eventId='+encodeURIComponent(eid)+'&deviceId='+encodeURIComponent(getDeviceId())+photoHostQs())
        .then(function(r){return r.json();})
        .then(function(d){
          if(d.error&&!d.photos) return; // S3 not configured, fall back to AI-generated code
          var photos=d.photos||[];
          var isHost=!!d.is_host;
          allSecs.forEach(function(sec,i){
            var idx=getSectionIndex(sec,allSecs);
            var secPhotos=photos.filter(function(p){return p.section_index===idx;});
            renderCloudPhotos(sec,secPhotos,isHost);
          });
        })
        .catch(function(e){console.warn('[OneDay] photo list failed',e);});
    }

    function wirePhotoUploads(){
      var allSecs=findPhotoSections();
      if(!allSecs.length) return;
      photoSections=allSecs;

      allSecs.forEach(function(sec,i){
        var idx=getSectionIndex(sec,allSecs);

        // Fix display:none file inputs WITHOUT cloning (preserve existing listeners)
        var fileInp=sec.querySelector('input[type="file"]');
        if(fileInp){
          if(getComputedStyle(fileInp).display==='none'){
            fileInp.style.cssText='position:absolute;opacity:0;width:1px;height:1px;overflow:hidden;';
          }
          // Mark so we don't wire twice
          if(fileInp.dataset.onedayWired) return;
          fileInp.dataset.onedayWired='1';
        } else {
          // Create a hidden file input if completely missing
          fileInp=document.createElement('input');
          fileInp.type='file';
          fileInp.accept='image/*';
          fileInp.multiple=true;
          fileInp.id='oneday-photo-input-'+idx;
          fileInp.style.cssText='position:absolute;opacity:0;width:1px;height:1px;overflow:hidden;';
          fileInp.dataset.onedayWired='1';
          sec.appendChild(fileInp);

          // Create upload button if no label found
          var existingLbl=sec.querySelector('label[class*="upload"],label[class*="btn"],button[class*="upload"],.upload-btn');
          if(!existingLbl){
            var btn=document.createElement('label');
            btn.setAttribute('for',fileInp.id);
            btn.textContent='+ Add Photos';
            btn.style.cssText='display:inline-block;padding:10px 20px;border-radius:8px;background:#111;color:#fff;cursor:pointer;font-size:14px;font-weight:600;margin-top:12px;';
            var grid=sec.querySelector('.oneday-photo-grid');
            if(grid) sec.insertBefore(btn,grid);
            else sec.appendChild(btn);
          }
        }

        // Add cloud upload listener on top of any existing ones
        fileInp.addEventListener('change',function(){
          ...
          files.forEach(function(file){
            ...
            fetch('/api/event-photos/presign',{
              method:'POST',
              headers:{'Content-Type':'application/json'},
              body:JSON.stringify({eventId:eid,sectionIndex:idx,contentType:file.type||'image/jpeg'})
            })
            .then(function(r){return r.json().then(function(j){if(!r.ok)throw new Error(j.error||'Upload failed');return j;});})
            .then(function(j){
              return fetch(j.uploadUrl,{method:'PUT',headers:{'Content-Type':file.type||'image/jpeg'},body:file})
                .then(function(r2){if(!r2.ok)throw new Error('Upload to storage failed');return j;});
            })
            .then(function(j){
              return fetch('/api/event-photos/register',{
                method:'POST',
                headers:{'Content-Type':'application/json'},
                body:JSON.stringify({eventId:eid,sectionIndex:idx,key:j.key,byteSize:file.size,contentType:file.type||'image/jpeg',deviceId:getDeviceId()})
              }).then(function(r){return r.json().then(function(jr){if(!r.ok)throw new Error(jr.error||'Register failed');return jr;});});
            })
            .then(function(){ loadCloudPhotos(); })
