...
(function(){
  function run(){
    ...
    function wireMessageButtons(){
      ...
      // Fallback: plain button text like "Post Message" with no id/class.
      sec.querySelectorAll('button').forEach(function(btn){
        if(btn.dataset.onedayWired) return;
        var t=(btn.textContent||'').trim().toLowerCase();
        if(!t) return;
        if(t.indexOf('post message')===-1&&t.indexOf('send')===-1&&t.indexOf('post')===-1) return;
        btn.dataset.onedayWired='1';
        btn.addEventListener('click',function(e){ e.preventDefault(); cloudSubmit(); });
      });
    }
    ...
    function ensureCountdownFallback(){
      ...
      var dm=text.match(/(jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:t(?:ember)?)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\s+\d{1,2},\s*\d{4}/i);
      if(!dm) return;
      var tm=text.match(/(\d{1,2}:\d{2}\s*[AP]M)/i);
      var target=new Date(dm[0] + (tm ? (' '+tm[1]) : ''));
