...
(function(){
  function run(){
    ...
    function renderRsvpList(rsvps, isHost){
    ...
    /** Templates often use onclick="handleRSVP(); showThanks()" — showThanks runs after handleRSVP returns while fetch is still in flight. Hide those blocks until the API succeeds. */
    function hideRsvpFalseSuccessUI(){
      document.querySelectorAll(
        '#rsvpThanks,#rsvpThankYou,#rsvpSuccess,#rsvpThank,#rsvpConfirmation,#rsvpConfirm,'+
        '[id*="rsvpThank"],[id*="RsvpThank"],[id*="rsvpConfirm"],[data-rsvp-thanks],[data-rsvp-success],'+
        '[class*="rsvp-success"],[class*="rsvp-thank"],[class*="rsvp-confirm"]'
      ).forEach(function(el){
      ...
      if(sec){
        sec.querySelectorAll('p,div,span,aside,h2,h3').forEach(function(el){
          if(el.querySelector&&el.querySelector('input,button,textarea,select,form')) return;
          var t=(el.textContent||'').trim();
          if(t.length>8&&t.length<220&&/rsvp.*received|thank you.*rsvp|your rsvp has been/i.test(t)){
            el.style.display='none';
            el.setAttribute('hidden','');
          }
        });
      ...
      fetch('/api/event-rsvps/list?eventId='+encodeURIComponent(eid)+'&deviceId='+encodeURIComponent(getDeviceId())+hostQs())
        ...
        .then(function(d){
          ...
          if(localId&&mine){
            ...
            var still=all.some(function(r){ return r.owned_by_me; });
            if(!still){
              localStorage.removeItem(rsvpDoneKey);
              if(btn){
              ...
              if(note) note.textContent='Guests can RSVP from any device — totals update live.';
            } else if(btn){
              btn.textContent="✅ You're on the list!";
              btn.disabled=true;
              btn.classList.add('rsvp-gone');
            }
            if(still&&note) note.textContent='Thanks! See you there.';
          } else if(note){
            note.textContent='Guests can RSVP from any device — totals update live.';
          }
          if(d.is_host){
            var noteH=document.getElementById('rsvpNote');
            if(noteH) noteH.textContent='Host mode — you can remove RSVPs from the list below.';
          }
        })
