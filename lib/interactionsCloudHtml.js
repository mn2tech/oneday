...
(function(){
  function run(){
    ...
    window.__onedayLoadMessages=loadMessages;
    document.addEventListener('oneday:messages-refresh', loadMessages);
    ...
    function applyPollUI(counts, myChoice){
      ...
      for(var i=0;i<counts.length;i++){
        ...
        if(el) {
          ...
          el.textContent=/\\bvotes?\\b/i.test(parentText.replace(el.textContent||'','')) ? String(cnt) : cnt+' vote'+(cnt!==1?'s':'');
        }
    ...
    function wirePollOptions(){
      ...
      for(var i=0;i<n;i++){
        (function(ix){
          var opt=document.getElementById('pollOpt'+ix);
          if(!opt||opt.dataset.onedayPollWired==='1') return;
          opt.dataset.onedayPollWired='1';
          opt.setAttribute('role', opt.getAttribute('role') || 'button');
          opt.setAttribute('tabindex', opt.getAttribute('tabindex') || '0');
          opt.addEventListener('click', function(){ window.vote(ix); });
          opt.addEventListener('keydown', function(e){
            if(e.key==='Enter'||e.key===' '){ e.preventDefault(); window.vote(ix); }
          });
        })(i);
      }
    }
    ...
    function syncInviteEmailPanel(sec,isHost){
      ...
      if(!el)return;
      if(isShareEventMode()||!isHost){
        el.style.display='none';
        el.setAttribute('data-oneday-share-hidden','1');
        return;
      }
      el.style.display='block';
      el.removeAttribute('data-oneday-share-hidden');
    }
    ...
    function isShareEventMode(){
      ...
      return /share event|scan to share photos|add photos & videos/.test(text);
    }
