...
(function(){
  function run(){
    ...
    function cloudHandleRSVP(){
      var localId=localStorage.getItem(rsvpIdKey)||'';
      if(localStorage.getItem(rsvpDoneKey)==='1'&&!localId){
      ...
      var body={eventId:eid,guestName:inp.name,adults:inp.adults,kids:inp.kids};
      ...
      .then(function(j){
        ...
        var note=document.getElementById('rsvpNote');
        if(note) note.textContent=isUpdate?'Your RSVP was updated.':'Thanks! You can edit this RSVP from this device.';
        loadRsvps();
      })
      .catch(function(err){
