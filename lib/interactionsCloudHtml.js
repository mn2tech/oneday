  ...
  function run(){
    ...
    function renderCloudPhotos(sec, photos, isHost){
      ...
      photos.forEach(function(p){
        ...
        wrap.style.cssText='position:relative;aspect-ratio:1;border-radius:8px;overflow:hidden;background:#f0f0f0;';
    ...
    function wirePhotoUploads(){
      ...
      allSecs.forEach(function(sec,i){
        ...
        // Fix hidden file inputs — change display:none to accessible hidden
        ...
        if(!fileInp){
          ...
          if(getComputedStyle(fileInp).display==='none'){
            fileInp.style.display='';
            fileInp.style.cssText='position:absolute;opacity:0;width:1px;height:1px;overflow:hidden;';
          }
        }

        // Wire upload label to our file input
        var lbl=sec.querySelector('label[for="photo-input-'+idx+'"],label[for="photoInput'+idx+'"],label.upload-btn,.upload-btn,button[onclick*="photo"],button[onclick*="Photo"]');
        if(lbl){
          // Re-target label to our input
          lbl.setAttribute('for', fileInp.id);
          // If it's a button not a label, wire click
          if(lbl.tagName==='BUTTON'){
            lbl.onclick=function(e){e.preventDefault();fileInp.click();};
          }
        } else {
          ...
          btn.style.cssText='display:inline-block;padding:8px 16px;border-radius:8px;background:#111;color:#fff;cursor:pointer;font-size:14px;font-weight:600;margin-top:8px;';
          sec.insertBefore(btn, sec.querySelector('.oneday-photo-grid')||null);
          sec.appendChild(btn);
        }

        // Remove old change listeners by cloning
        var newInp=fileInp.cloneNode(false);
        fileInp.parentNode.replaceChild(newInp,fileInp);
        fileInp=newInp;
        if(lbl&&lbl.tagName==='LABEL') lbl.setAttribute('for',fileInp.id);
