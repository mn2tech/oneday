  ...
  document.addEventListener('DOMContentLoaded', function(){
    setTimeout(function(){
      ...
      function renderPollBars(counts, myChoice){
        ...
        var bars=poll.querySelectorAll('.poll-bar');
        if(bars.length<2) bars=poll.querySelectorAll('[id*="Bar"], [id*="bar"]');
        if(bars.length>=2){
        ...
        var opts=poll.querySelectorAll('.poll-option, [id^="opt"], [id^="pollOpt"]');
      ...
      if(pollEl){
        ...
        var opts=pollEl.querySelectorAll('.poll-option, [id^="opt"], [id^="pollOpt"]');
      ...
      function findMsgList(){
        return document.querySelector('#msgList, #messageList, #msg-list, [id*="msgList"], [class*="msg-list"], [class*="message-list"]');
      }
      function findMsgInputs(){
        var ta=document.querySelector('#msgText')||document.querySelector('section textarea, [class*="message"] textarea');
        var na=document.querySelector('#msgName')||document.querySelector('section input[type="text"], [class*="message"] input[type="text"]');
        return {ta:ta, na:na};
      }
