...
- Preserve localStorage key pattern for photos only: eventId = (location.pathname.split('/').pop()||'event').slice(0,30); keys photos_\${eventId}_0, photos_\${eventId}_1. On OneDay hosted pages, messages / RSVP / poll are NOT stored in localStorage (Supabase: event_messages, event_rsvps, event_poll_votes); do not add messages_*, rsvps_*, poll_* reads/writes.
...
- If the Photo Wall exists and has no working upload: use a <label for="photo-input-[n]"> styled as a button to trigger the file input natively. File input: id="photo-input-[n]" accept="image/*,video/mp4,video/webm,video/quicktime" multiple style="position:absolute;opacity:0;width:1px;height:1px;overflow:hidden;" — NEVER use display:none on file inputs as it blocks change events. The OneDay shared media engine will wire uploads and render photos/videos.
