...
3. Photo & Video Wall — For full invitations, use 2 sections with event-appropriate labels (e.g. Ceremony & Reception for weddings, Celebration & Fun for birthdays). For Share Event pages, use exactly 1 media upload section to avoid guest confusion. Each section MUST include ALL of the following:
...
7. (Conferences only) Speakers Section — cards with speaker name, title, topic. Use the speakers provided or create placeholder cards.
8. (Conferences only) Resource Hub — a clean list section where links to slides, recordings, and resources can be added. Show as styled cards with a title, description, and link button.`;
...
function buildUserPrompt(prompt, eventDateTime, eventMeta) {
  ...
  const shareInstruction = eventMeta?.eventMode === 'share'
    ? '\n\nEVENT MODE: SHARE EVENT. Create an event-day signboard and photo/video sharing page, not a full invitation. Make QR-code scanning and media upload the primary experience. The guest flow must be: scan the visible event QR code, enter name, write a short congratulations note/wish for the host, then upload photos or videos. Include only: a bold signboard-style hero, event name/date/host if provided, a visible QR code/signboard area, a large "Scan to Share Photos & Videos" / "Leave a Wish, Then Upload" call-to-action, a simple greeting/wish step, and one single media wall upload section. Do NOT create multiple upload sections. Do NOT include RSVP, schedule, itinerary, dress code, venue details, full message wall, guest book, guest count, attendee registration, "Who is coming", poll/voting, meal counts, attendance forms, or invitation acceptance language. Use concise wording suitable for a printed signboard QR code.'
    : '';
