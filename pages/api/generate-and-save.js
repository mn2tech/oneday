...
3. Photo Wall — 2 sections with event-appropriate labels (e.g. Ceremony & Reception for weddings, Celebration & Fun for birthdays). Each section: use a <label> element styled as a button that has a "for" attribute pointing to the file input id — this opens the picker natively with NO JavaScript needed. File input: id="photo-input-[section]" accept="image/*" multiple style="position:absolute;opacity:0;width:1px;height:1px;overflow:hidden;" — NEVER use display:none on file inputs as it blocks change events. On change event via addEventListener: FileReader→base64→localStorage key "photos_[eventId]_[section]", imgs with object-fit:cover height:180px in CSS grid (repeat(auto-fill,minmax(150px,1fr))), × remove button per photo (hidden if locked), max 20 photos/3MB per section
